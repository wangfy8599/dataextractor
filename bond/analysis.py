...
def format_data(df):
    ...
    df["辰星双底"] = df["转债溢价率"].astype(float) + df["转股溢价率"].astype(float)

    return df


def write_report(df_1, df_2, df_3):
...
def main():
    with open(r"D:\github\dataextractor\bond\2022\10\28.html", "r") as f:
        ...
        df_1 = df_1.query(""" 转债价格 < 125 and not (转债名称.str.contains("\\*")) """).sort_values(by=['辰星双底'], ascending=True).head(30)
        df_1.reset_index(drop=True, inplace=True)
        df_1.index = df_1.index + 1

        # 自选
        df_2 = df_all[df_all["转债代码"].isin(read_my_list())]
        df_2 = df_2.sort_values(by=['辰星双底'], ascending=True)
        df_2.reset_index(drop=True, inplace=True)
        df_2.index = df_2.index + 1

        # 观察
        df_3 = df_all[df_all["转债代码"].isin(read_watch_list())]
        df_3 = df_3.query(""" 转债价格 < 130 """).sort_values(by=['辰星双底'], ascending=True)
        df_3.reset_index(drop=True, inplace=True)
        df_3.index = df_3.index + 1
        write_report(df_1, df_2, df_3)
