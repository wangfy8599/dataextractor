...
def generate_report():
    ...
    df_1 = df_1.query(""" 利润同比>-1000 """)
    df_1 = df_1.sort_values(by=['利润同比'], ascending=False)
    df_1.reset_index(drop=True, inplace=True)
    df_1.index = df_1.index + 1

    df_2 = df_all[df_all["转债代码"].isin(config.read_watch_list())]
    df_2 = df_2.query(""" 利润同比>-1000 """)
    df_2 = df_2.sort_values(by=['利润同比'], ascending=False)
    df_2.reset_index(drop=True, inplace=True)
    df_2.index = df_2.index + 1

    df_3 = df_all.query(""" 转债价格 < 125 and 股价 > 3 and 剩余年限 > 1.0 and PB > 1.0 and 营收同比>0 and 利润同比>0 and ROE>5 """)
    df_3 = df_3.sort_values(by=['利润同比'], ascending=False)
    df_3.reset_index(drop=True, inplace=True)
    df_3.index = df_3.index + 1

    df_4 = df_all.sort_values(by=['辰星三低'], ascending=True).head(3000)
    df_4 = df_4.query(""" 转债价格 < 125 and PB>1 and 剩余年限>1 and 利润同比>0 """).head(100)
    df_4.reset_index(drop=True, inplace=True)
    df_4.index = df_4.index + 1

    df_5 = df_all.sort_values(by=['辰星双低'], ascending=True).head(3000)
    df_5 = df_5.query(""" 转债价格 < 125 and 剩余年限>1 and 利润同比>0 """).head(100)
    df_5.reset_index(drop=True, inplace=True)
    df_5.index = df_5.index + 1

    df_6 = df_all.sort_values(by=['利润同比'], ascending=False).head(3000)
    df_6 = df_6.query(""" 转债价格 < 125 and 利润同比>0 """).head(100)
    df_6.reset_index(drop=True, inplace=True)
    df_6.index = df_6.index + 1

    write_stock_2_report([df_0, df_1, df_2, df_3, df_4, df_5, df_6], constants.bond_yjkb_report_file)
